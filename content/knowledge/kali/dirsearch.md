...
- 工具位置：`/usr/bin/dirsearch`
- 当前状态：已在 Kali SSH 环境中确认
- 来源线索：`比赛环境手册/tool/dirsearch.md`

## 比赛中怎么用

dirsearch 的核心价值是：Web 目录与文件爆破工具，适合快速发现后台、备份文件、接口路径。 比赛时先确认输入文件、目标地址、凭据和权限，再逐步增加参数。

## 常用命令

```bash
dirsearch -u http://target/ -w wordlist.txt -e php,txt,zip
```
...
Options:
  ...
  Dictionary Settings:
    ...
    --remove-extensions
                        Remove extensions in all paths (e.g. admin.php -> admin)
    --prefixes=PREFIXES
