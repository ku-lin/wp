...
- 使用领域：比赛环境 / 网络
- 主要用途：OpenVPN 客户端，用于连接比赛 VPN、导入 ovpn 配置。
- 工具位置：`/usr/sbin/openvpn`
- 当前状态：已在 Kali SSH 环境中确认
- 来源线索：`比赛环境手册/tool/openvpn.md`

## 比赛中怎么用

openvpn 的核心价值是：OpenVPN 客户端，用于连接比赛 VPN、导入 ovpn 配置。 比赛时先确认输入文件、目标地址、凭据和权限，再逐步增加参数。
