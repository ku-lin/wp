...
- 工具位置：`D:\tool\CTF\Web\sqlmap-dev\sqlmap.py`
- 当前状态：已在 D:\tool 中确认位置
- 来源线索：`D:\tool`

## 比赛中怎么用

sqlmap-dev 的核心价值是：自动化 SQL 注入检测与利用工具，可枚举库表、dump 数据、读写文件。 比赛时先确认输入文件、目标地址、凭据和权限，再逐步增加参数。
...
- 目标输入：`-u`、`-r`、`-m`、`--data`、`--cookie`。
- 测试控制：`--level`、`--risk`、`--technique`、`-p`、`--skip`、`--dbms`、`--tamper`。
- 枚举导出：`--dbs`、`--tables`、`--columns`、`--dump`、`-D`、`-T`、`-C`。
...
```text
        ___
       __H__
 ___ ___[)]_____ ___ ___  {1.10.3.5#dev}
|_ -| . [']     | .'| . |
|___|_  [,]_|_|_|__,|  _|
...
Options:
  ...
  Request:
    ...
    --http1.0           Use HTTP version 1.0 (old)
