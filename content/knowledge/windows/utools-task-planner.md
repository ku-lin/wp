...
- 工具位置：`D:\tool\uTools\utools-task-planner`
- 当前状态：已在 D:\tool 中确认位置
- 来源线索：`D:\tool`

## 比赛中怎么用

utools-task-planner 的核心价值是：任务规划插件项目，和比赛工具关系较弱。 比赛时先确认输入文件、目标地址、凭据和权限，再逐步增加参数。
...
- 不是 CTF 攻防工具；目录内含 Android 构建依赖，不纳入参数展开。
